...
class ProductGatlingTest extends Simulation {
    ...
    val authorization_header = "Basic " + Base64.getEncoder.encodeToString("shopapp:my-secret-token-to-change-in-production".getBytes(StandardCharsets.UTF_8))
    ...
    val scn = scenario("Test the Product entity")
        ...
        .repeat(2) {
            ...
            .post("/api/products")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "icon":"SAMPLE_TEXT", "salePrice":null, "marketPrice":null, "originPrice":null, "discount":null, "saleCount":"0", "isNew":null, "goodRemark":"0", "commentCount":"0"}""")).asJSON
            .check(status.is(201))
